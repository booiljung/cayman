...
다트의 타입 안전성은 `if (nonbooleanValue)` 또는 `assert (nonbooleanValue)`와 같은 코드를 사용할 수 없다는 것을 의미합니다. 대신 다음과 같이 명시 적으로 값을 확인해야 합니다.
...
스프레드 연산자 사용에 대한 자세한 내용 및 예는 스프레드 연산자 제안을 참조하십시오.
