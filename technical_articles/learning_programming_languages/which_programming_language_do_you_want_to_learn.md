...
소프트웨어 개발자를 하려면 프로그래밍 언어를 배워야 합니다. 누구에게나 처음 배우는 프로그래밍 언어가 있습니다. 저는 80년대에 배웠고, BASIC이 첫 프로그래밍 언어였습니다. 첫 프로그래밍 언어로 어느 언어를 선택해야 할까요?

프로그래밍 언어를 처음 배우게 되면, 개발 도구를 설치하고, 가장 먼저 `Hello, world!`를 출력하는 코드를 배웁니다. 이 `Hello, world!`는 1978년에 브라이언 커니핸과 데니스 리치가 쓴 `The Programming Language` 교재의 첫 번째 예제였고, 모든 프로그래밍 언어 학습서의 첫번째 예제로 굳어지게 되었습니다.

어느 언어의 `Hello, world!`예제가 가장 쉬울까요? 다양한 언어로 `Hello, world!`를 출력하는 코드를 보겠습니다.
